...
class TSNE:
    def __init__(self, data, ydim=2, num_iterations=100, learning_rate=100, perp=30):
        self.data = data
        self.ydim = ydim
    ...
    def run(self) -> np.array:
        ...
        for t in range(self.num_iterations):
            ...
            if t < 50:
                ...
                grad = self.gradient(P=4 * P, Q=Q, y=map_points[-1])
            else:
                grad = self.gradient(P=P, Q=Q, y=map_points[-1])
            # Alacsony dimenziós pontok t-edik iterációjának meghatározása:
            new_mappoints = map_points[-1] - self.learning_rate * grad + \
                            self.momentum(t) * (map_points[-1] - map_points[-2])
            map_points.append(new_mappoints)

            # Az iteráció elakadásának elkerülése érdekében minden tizedik iterációnál megnlzzük,
            # hogy a síkbeli pontok hasonlóságának vlaószínűsége nem lett-e túl kicsi
            if t % 10 == 0:
    ...
    @staticmethod
    def p_i_j(dists: np.array, sigma: np.array) -> np.array:
        """
    ...
    @staticmethod
    def p_j_i(dists: np.array, sigma: np.array) -> np.array:
        """
        Páronkénti szomszédságot / hasonlóságot mérő
    ...
    def p_joint(self) -> np.array:
        """
        :return: szimmetrikus valószínűségek kiszámolása
        """
        N = self.data.shape[0]
        dists = self.pairwise_distances(data=self.data)
        sigmas = self.find_sigmas(dists=dists, perplexity=self.perp)
        p_cond_1 = self.p_i_j(dists=dists, sigma=sigmas)
        p_cond_2 = self.p_j_i(dists=dists, sigma=sigmas)
        return (p_cond_1 + p_cond_2) / (2.*N)

    @staticmethod
    def perp(p_cond: np.array) -> float:
        # Perp kiszámolása (3) szerint
        entropy = -np.sum(p_cond * np.log2(p_cond), axis=1)
        return 2 ** entropy

    def find_sigmas(self, dists: np.array, perplexity: int) -> np.array:
        """
        ...
        for i in range(dists.shape[0]):
            func = lambda sig: self.perp(self.p_i_j(dists=dists[i:i + 1, :],
                                                      sigma=np.array([sig])))
            found_sigmas[i] = self.binary_search(func=func, goal=perplexity)
        return found_sigmas

    @staticmethod
    def binary_search(func, goal: int, tol=1e-10, max_iters: int = 1000, lowb: int = 1e-20,
    ...
    def q_i_j(self, y: np.array) -> np.array:
        """
    ...
    def gradient(self, P: np.array, Q: np.array, y: np.array) -> np.array:
        """
